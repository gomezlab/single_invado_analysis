...
for (@folders) {
    ...
    $file_name =
      &write_data_file("$_/Centroid_dist_from_edge", "$_/Average_adhesion_signal", "$_/Variance_adhesion_signal");
    foreach (split(/\s/, $cfg{file_ext})) {
    ...
    if ($opt{debug}) {
        ...
        foreach (0 .. $#data) {
            if (scalar(@{ $data[$_] } != $data_size)) {
                print "Problem with files: $_[0], $_[$_], the number of entries do ", "not match, they are: ",
                  scalar(@{ $data[0] }), " and ", scalar(@{ $data[$_] }), "\n";
            }
            if (${ $data[$_] }[0] =~ /\s+/) {
...
sub write_data_file {
    ...
    if ($opt{debug}) {
        open OUTPUT, ">$_/" . join("_", @header_line) . "_data_file"
          or die "Unable to open the output file";
...
sub get_title_and_labels {
    ...
    my %t_hash    = (
        cent_area => [
            "Centroid Distance from Cell Edge vs. Area of Identified Adhesions",
            "Centroid Distance from Cell Edge",
            "Area of Identified Adhesions",
        ],
        cent_sig => [
            "Centroid Distance from Cell Edge vs. Average Adhesion Signal",
            "Centroid Distance from Cell Edge",
...
sub get_output_type {
    ...
    if (not(defined($f_types_to_terms{$file_type}))) {
        die "In file: $out_file\n", "Can't match determined file extension - $file_type - to a gnuplot ",
          "term name, possible file extensions: ", join(", ", keys %f_types_to_terms), "\n";
    }
