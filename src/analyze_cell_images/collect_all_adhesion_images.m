%COLLECT_ALL_ADHESION_IMAGES    wrapper script used to setup the directory
%                               structure and execute all the needed
%                               commands to collect all the adhesion images
%

base_data_folder = fullfile('..','..','data','sample_images');
base_results_folder = fullfile('..','..','results','sample_images_low_thresh');

cell_mask_image_folder = fullfile(base_data_folder,'image_stacks','cell_mask_images');
cell_mask_file_prefix = '';

adhesion_protein_image_folder = fullfile(base_data_folder,'image_stacks','adhesion_protein');
...
for i = 1:stack_num
    ...
    for j = 4:image_set_cell_number
        ...
        image_data.pixel_size = importdata(fullfile(base_data_folder,'pixel_size.txt'));
        ...
        image_data.results_directory = fullfile(base_results_folder,'individual_pictures',image_data.padded_time_point_num,image_data.padded_cell_num);
        image_data.interesting_directory = fullfile(base_results_folder,'many_adhesions',image_data.padded_time_point_num,image_data.padded_cell_num);

        image_data.original_focal_image = normalize_grayscale_image(imread(adhesion_protein_stack_location,j));

        if (exist(fullfile(image_data.results_directory,'cell_mask.png'),'file'))
            image_data.cell_mask = imread(fullfile(image_data.results_directory,'cell_mask.png'));
            image_data.cell_edge = bwperim(image_data.cell_mask);
        else
            image_data.cell_mask = create_cell_mask_image(cell_mask_stack_location,j,image_data.results_directory);
            image_data.cell_edge = bwperim(image_data.cell_mask);
        end
        ...
        image_data.identified_adhesions = find_watershed_adhesions(image_data.original_focal_image,image_data.watershed_edges,image_data.cell_mask);
        ...
        if (not(exist(image_data.results_directory,'dir')))
            mkdir(image_data.results_directory);
        end
        imwrite(image_data.focal_edge_highlights,fullfile(image_data.results_directory,'focal_highlights.png'));
        imwrite(image_data.composite_image,fullfile(image_data.results_directory,'comp.png'));
