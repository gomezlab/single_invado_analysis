...
if (not(isfield(image_data,'focal_edge_highlights')))
    focal_edge_highlights = image_data.focal_image;
else
...
image_data.focal_edge_highlights(full_cell_centroid(2)-5:full_cell_centroid(2)+5,full_cell_centroid(1)-5:full_cell_centroid(1)+5,1) = 1;
...
for k = 1:2:size(centroid_stats,2)
    ...
    image_data.focal_edge_highlights(row_range,col_range,1:3) = color;
end
