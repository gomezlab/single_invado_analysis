...
field_size = [max_ad_size^2 - max_ad_size*(min_ad_size - 1) + 2*ad_padding + ad_padding*(max_ad_size - min_ad_size + 1), ...
              ceil(2*ad_padding + image_width + max_ad_size)];
...
for j = 2:(image_number-1)
    ...
    for i = min_ad_size:max_ad_size
        ...
        row_range = 1:size(temp_ad,1);
        row_shift = floor(ad_padding + ad_padding * (i - min_ad_size) + max_ad_size * (i - min_ad_size) + max_ad_size/2);
        row_range = row_range + row_shift;
