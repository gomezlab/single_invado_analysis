...
my $run_file = "IAS_webapp.$opt{ID}.run";
...
$oldest_data{ID} = basename($oldest_data{upload_folder});
...
sub send_email {
	...
	my $from_str = "\"From: noreply\@$hostname (IAS Notification)\"";
...
sub setup_exp {
	...
	my $command = "./setup_results_folder.pl -cfg $oldest_data{cfg_file} > /dev/null 2>/dev/null";
