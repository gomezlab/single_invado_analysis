...
sub create_all_matlab_commands {
    ...
    foreach (0..$#image_files) {
		my $file = $image_files[$_];
		my $affine_matrix = catfile(dirname($file), 'affine_matrix_cascade.csv');
        my $extra_opt = "";
		if (-e $affine_matrix) {
            $extra_opt .= ",'transform_file','$affine_matrix'";
		} else {
			if ($_ != 0) {
				die "Expected to find cascaded affine matrices for all files " . 
					"past the first, unable to find: $affine_matrix";
			}
		}

		$matlab_code[0] .= "apply_registration('$file'$extra_opt)\n";
    }
